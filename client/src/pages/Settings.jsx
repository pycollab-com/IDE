...
export default function Settings({ user: currentUser, onLogout }) {
  ...
  const hydrateProfileForm = (user) => {
    ...
    if (user.profile_picture_path) {
      if (user.profile_picture_path.startsWith("http") || user.profile_picture_path.startsWith("data:")) {
        setPfpPreview(user.profile_picture_path);
      } else {
        setPfpPreview(`${API_BASE}${user.profile_picture_path}`);
      }
    }
  };
  ...
  const handlePfpUpload = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("user", JSON.stringify(res.data));
      setPfpPreview(`${API_BASE}${res.data.profile_picture_path}`);
