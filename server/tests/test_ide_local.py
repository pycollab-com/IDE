...
def test_hosted_login_relays_password_sign_in():
    with tempfile.TemporaryDirectory() as temp_dir:
        os.environ["PYCOLLAB_IDE_HOME"] = os.path.join(temp_dir, "ide-home")
        ...
        class FakeResponse:
            ...
            def read(self):
                return b'{"access_token":"token","user":{"id":1,"username":"adam"}}'
        ...
        with patch("server.ide_app.urlopen", side_effect=fake_urlopen):
            proxied = client.post(
                "/ide/auth/login",
                json={"username": "adam", "password": "secret"},
            )

        assert proxied.status_code == 200
        assert proxied.json()["access_token"] == "token"
        assert captured["url"].endswith("/auth/login")
        assert captured["method"] == "POST"
        assert captured["body"] == "username=adam&password=secret"
